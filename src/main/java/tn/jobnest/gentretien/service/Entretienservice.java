...
public class Entretienservice implements Icrud<Entretien> {
    ...
    public void ajouter(Entretien entretien) throws SQLException {
        String sql = "INSERT INTO entretien (date_entretien, heure_debut, heure_fin, type_entretien, lieu, lien_visio, statut, note_recruteur, date_creation, id_recruteur, id_offre) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public void update(Entretien entretien) throws SQLException {
        String sql = "UPDATE entretien SET date_entretien = ?, heure_debut = ?, heure_fin = ?, type_entretien = ?, lieu = ?, lien_visio = ?, statut = ?, note_recruteur = ?, date_creation = ?, id_recruteur = ?, id_offre = ? WHERE id_entretien = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setDate(1, entretien.getDateEntretien());
            ps.setTime(2, entretien.getHeureDebut());
            ps.setTime(3, entretien.getHeureFin());
    ...
    public List<String> getParticipants(int idEntretien) throws SQLException {
        ...
        String sqlCand = "SELECT c.prenom, c.nom " +
                "FROM candidat c " +
