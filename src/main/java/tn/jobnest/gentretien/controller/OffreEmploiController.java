...
public class OffreEmploiController {
                ...
                Platform.runLater(() -> {
                    if (count > 0) {
                        ...
                        ScaleTransition pulse = new ScaleTransition(
                                Duration.millis(200), notifBadgeLabel);
    ...
    private VBox buildPage(int pageIndex) {
        ...
        Platform.runLater(() -> {
            ...
            if (filteredData.isEmpty()) {
                ...
                empty.setStyle(
                        "-fx-font-size:14px; -fx-text-fill:#94A3B8; -fx-padding:40;");
    ...
    // ════════════════════════════════════════════════════════════════
    //  CARTE — statut | titre | entreprise | nom recruteur | actions
    // ════════════════════════════════════════════════════════════════
    private VBox createCard(OffreEmploi o) {
        ...
        card.setStyle(
                "-fx-background-color: white; -fx-background-radius: 14px;" +
                        "-fx-border-radius: 14px; -fx-border-color: #E2E8F0;" +
                        "-fx-border-width: 1.5px; -fx-padding: 20px;" +
                        "-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.08), 12, 0, 0, 4);"
        );
        card.setOnMouseEntered(e -> card.setStyle(
                "-fx-background-color: white; -fx-background-radius: 14px;" +
                        "-fx-border-radius: 14px; -fx-border-color: #BFDBFE;" +
                        "-fx-border-width: 1.5px; -fx-padding: 20px;" +
        ...
        titre.setStyle(
                "-fx-font-size:17px; -fx-font-weight:bold; -fx-text-fill:#1E3A5F;"
        );
        titre.setWrapText(true);

        // ── Entreprise ────────────────────────────────────────────
        Label entreprise = new Label(
                "🏢  " + (o.getEntreprise() != null ? o.getEntreprise() : "—"));
        entreprise.setStyle("-fx-font-size:13px; -fx-text-fill:#475569;");

        // ── Nom complet du recruteur (depuis RecruteurService) ────
        String nomRecruteur = service.getNomRecruteur(o.getIdRecruteur());
        Label recruteur = new Label("👤  " + nomRecruteur);
        recruteur.setStyle(
        ...
        HBox row2 = new HBox(8, ouvert ? fermerBtn : republierBtn, deleteBtn);
        ...
        card.getChildren().addAll(
                statusBadge, titre, entreprise, recruteur, sep, actionsBox);
        return card;
    }
    ...
    private void confirmerFermeture(OffreEmploi o) {
        ...
        if (result.isPresent() && result.get() == btnFermer) {
            try {
                ...
                ok.setContentText(
                        "✅  L'offre « " + o.getTitre() + " » a été fermée avec succès.");
                ok.showAndWait();
            } catch (SQLException e) {
    ...
    private void lancerRepublication(OffreEmploi o) {
        try {
            ...
            if (offreModifiee != null) {
                ...
                ok.setGraphic(null);
                ok.setContentText(
                        "✅  L'offre « " + offreModifiee.getTitre() +
                                " » est à nouveau ouverte aux candidatures !");
                ok.showAndWait();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void confirmerSuppression(OffreEmploi o) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try { service.supprimerOffre(o.getIdOffre()); chargerOffres(); }
