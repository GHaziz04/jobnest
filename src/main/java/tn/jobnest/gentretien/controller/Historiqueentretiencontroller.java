...
public class Historiqueentretiencontroller {
    ...
    @FXML private ComboBox<String> comboStatutHistorique;  // Tous / Réalisés / Annulés
    @FXML private ComboBox<String> comboFeedback;          // Tous / Avec / Sans
    ...
    /** Contient TOUS les entretiens réalisés + annulés du recruteur */
    ...
    public void initialize() {
        ...
        // Listeners pour filtrage dynamique
    ...
    // =====================================================================
    // CHARGEMENT : réalisés + annulés uniquement
    // =====================================================================
    ...
    private void filterAndDisplay() {
        ...
        for (Entretien e : allHistorique) {
            try {
                ...
                boolean matchStatut;
                if ("🏁 Réalisés uniquement".equals(selStatut)) {
                    matchStatut = estRealise;
                } else if ("❌ Annulés uniquement".equals(selStatut)) {
                    matchStatut = estAnnule;
                } else {
                    matchStatut = true;
                }
                ...
                if ("Avec feedback".equals(selFeedback)) {
                    // Seuls les réalisés avec feedback
                    matchFeedback = estRealise && hasFeedback;
                } else if ("Sans feedback".equals(selFeedback)) {
                    // Réalisés sans feedback + annulés (ils n'ont jamais de feedback)
    ...
    // =====================================================================
    // CRÉATION CARTE
    // Réalisé  → bordure verte (avec feedback) ou orange (sans feedback)
    //            → bouton 💬 Ajouter Feedback ou ✅ Feedback enregistré
    // Annulé   → bordure rouge → bouton 🔄 Réorganiser
    // =====================================================================
    private Node createHistoriqueCard(Entretien e, List<String> participants,
        ...
        long duree = (e.getHeureDebut() != null && e.getHeureFin() != null)
                ? TimeUnit.MILLISECONDS.toMinutes(e.getHeureFin().getTime() - e.getHeureDebut().getTime())
                : 0;
        ...
        if (estAnnule) {
            ...
            if (hasFeedback) {
                ...
                btnAction.setPrefHeight(42);
                btnAction.setStyle(
                        "-fx-background-color: linear-gradient(135deg, #2563EB, #1D4ED8);" +
                                "-fx-text-fill: white;" +
                                "-fx-font-weight: 800;" +
                                "-fx-font-size: 13px;" +
                                "-fx-background-radius: 10px;" +
        ...
        actionsContainer.getChildren().addAll(typeBadge, btnAction);
    ...
    /**
     * Réorganise un entretien annulé : ouvre le formulaire en mode réorganisation.
     * Après enregistrement, le statut repasse à 'proposé' et l'entretien
     * disparaît de l'historique pour réapparaître dans entretien-view.
     */
    private void reorganiserEntretien(Entretien e) {
        try {
            ...
            stage.showAndWait();
            // Après réorganisation, l'entretien passe à 'proposé'
            // donc il disparaît de l'historique → on rafraîchit
            chargerHistorique();
    ...
    /**
     * Ouvre le formulaire de feedback pour un entretien réalisé.
     */
    ...
    private void ouvrirFeedbacks(ActionEvent event) {
        naviguerVers("/tn/jobnest/gentretien/feedback-interface.fxml", "JobNest - Gestion des Feedbacks", event);
    }
