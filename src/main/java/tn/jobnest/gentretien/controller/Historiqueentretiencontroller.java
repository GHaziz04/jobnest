...
public class Historiqueentretiencontroller {
    ...
    public void initialize() {
        ...
        comboStatutHistorique.setItems(FXCollections.observableArrayList("Tous (réalisés + annulés)", "🏁 Réalisés uniquement", "❌ Annulés uniquement"));
        comboStatutHistorique.setValue("Tous (réalisés + annulés)");
        comboFeedback.setItems(FXCollections.observableArrayList("Tous les feedbacks", "Avec feedback", "Sans feedback"));
    ...
    private void afficherToast(Notification notif) {
        try {
            if (historiqueVBox.getScene() != null) {
                Stage stage = (Stage) historiqueVBox.getScene().getWindow();
    ...
    private void chargerHistorique() {
        try {
            ...
            if (nbExpires > 0) System.out.println("[Auto-annulation] " + nbExpires + " entretien(s) → annulé(s)");
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur BD", "Impossible de charger l'historique : " + e.getMessage());
        }
    }
    ...
    private void updateStats() {
        ...
        long avecFb = allHistorique.stream()
                ...
                .filter(e -> { try { return feedbackService.feedbackExists(e.getIdEntretien()); } catch (SQLException ex) { return false; } })
                .count();
    ...
    private void filterAndDisplay() {
        ...
        if (allHistorique == null || allHistorique.isEmpty()) { afficherMessageVide("Aucun entretien dans l'historique pour le moment."); return; }
        ...
        for (Entretien e : allHistorique) {
            try {
                ...
                if ("Avec feedback".equals(selFeedback)) matchFeedback = estRealise && hasFeedback;
                else if ("Sans feedback".equals(selFeedback)) matchFeedback = (estRealise && !hasFeedback) || estAnnule;
                else matchFeedback = true;
                if (matchSearch && matchType && matchStatut && matchFeedback) {
                    historiqueVBox.getChildren().add(createHistoriqueCard(e, participants, titreOffre, hasFeedback));
                    auMoinsUn = true;
                }
            } catch (SQLException ex) { System.err.println("Erreur chargement historique entretien #" + e.getIdEntretien()); }
        }
    ...
    private Node createHistoriqueCard(Entretien e, List<String> participants, String titreOffre, boolean hasFeedback) {
        HBox card = new HBox(15);
        card.getStyleClass().add("card");
        card.setPrefHeight(130);
        boolean estAnnule = "annulé".equals(e.getStatut());
        String couleurBordure = estAnnule ? "#DC2626" : (hasFeedback ? "#059669" : "#F97316");
        card.setStyle("-fx-border-color: " + couleurBordure + "; -fx-border-width: 0 0 0 5; -fx-background-color: white; -fx-background-radius: 12px; -fx-border-radius: 12px; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.07), 10, 0, 0, 3);");

        String participantName = participants.isEmpty() ? "Candidat" : participants.get(0);
        String initials = participantName.chars().filter(Character::isUpperCase).mapToObj(c -> String.valueOf((char) c)).collect(Collectors.joining());
        ...
        Circle avatar = new Circle(25); avatar.getStyleClass().add("avatar");
        Label avatarLabel = new Label(initials); avatarLabel.setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: white;");
        StackPane avatarStack = new StackPane(); avatarStack.getChildren().addAll(avatar, avatarLabel);
        VBox avatarBox = new VBox(avatarStack); avatarBox.setAlignment(javafx.geometry.Pos.CENTER); avatarBox.setPrefWidth(70);

        VBox details = new VBox(7); details.setPrefWidth(430); details.setPadding(new Insets(5, 0, 5, 0));
        Label offreLabel = new Label(titreOffre); offreLabel.setStyle("-fx-font-size: 17px; -fx-font-weight: bold; -fx-text-fill: #1E3A5F;");
        String participantsText = participants.size() > 1 ? "👥 " + String.join(", ", participants) : "👤 " + (participants.isEmpty() ? "Aucun candidat" : participants.get(0));
        Label participantsLabel = new Label(participantsText); participantsLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #2c3e50; -fx-font-weight: 500;");
        String dateStr = (e.getDateEntretien() != null) ? e.getDateEntretien().toLocalDate().format(DateTimeFormatter.ofPattern("EEE dd MMM yyyy")) : "Date non définie";
        String heureStr = (e.getHeureDebut() != null) ? e.getHeureDebut().toLocalTime().format(DateTimeFormatter.ofPattern("HH:mm")) : "--:--";
        long duree = (e.getHeureDebut() != null && e.getHeureFin() != null) ? TimeUnit.MILLISECONDS.toMinutes(e.getHeureFin().getTime() - e.getHeureDebut().getTime()) : 0;
        Label dateTimeLabel = new Label("📅 " + dateStr + " à " + heureStr + (duree > 0 ? " (" + duree + " min)" : ""));
        dateTimeLabel.setStyle("-fx-font-size: 12px; -fx-text-fill: #7f8c8d;");
        Label lieuLabel = "présentiel".equals(e.getTypeEntretien())
                ? new Label("📍 " + (e.getLieu() != null && !e.getLieu().isEmpty() ? e.getLieu() : "Lieu non défini"))
                : new Label("🔗 Entretien en visioconférence");
        ...
        if (e.getNoteRecruteur() != null && !e.getNoteRecruteur().isEmpty()) {
            Label noteLabel = new Label("📝 " + e.getNoteRecruteur()); noteLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: #e67e22; -fx-font-style: italic;");
            details.getChildren().add(noteLabel);
        }

        VBox actionsContainer = new VBox(10); actionsContainer.setAlignment(javafx.geometry.Pos.CENTER_RIGHT); actionsContainer.setPrefWidth(230); actionsContainer.setPadding(new Insets(5, 0, 5, 0));
        Label typeBadge = new Label("présentiel".equals(e.getTypeEntretien()) ? "🏢 Présentiel" : "💻 Visio");
        typeBadge.setStyle("-fx-background-color: " + ("présentiel".equals(e.getTypeEntretien()) ? "#EDE9FE" : "#DBEAFE") + "; -fx-text-fill: " + ("présentiel".equals(e.getTypeEntretien()) ? "#7C3AED" : "#1D4ED8") + "; -fx-font-size: 11px; -fx-font-weight: 700; -fx-padding: 3 10 3 10; -fx-background-radius: 20px;");

        Button btnAction;
        if (estAnnule) {
            btnAction = new Button("🔄 Réorganiser"); btnAction.setPrefWidth(165); btnAction.setPrefHeight(42);
            btnAction.setStyle("-fx-background-color: linear-gradient(135deg, #ffffff, #EA580C); -fx-text-fill: white; -fx-font-weight: 800; -fx-font-size: 13px; -fx-background-radius: 10px; -fx-cursor: hand; -fx-effect: dropshadow(gaussian, rgb(37,99,235), 8, 0, 0, 3);");
            btnAction.setOnAction(ev -> reorganiserEntretien(e));
        } else {
            if (hasFeedback) {
                btnAction = new Button("✅ Feedback enregistré"); btnAction.setPrefWidth(185); btnAction.setPrefHeight(42); btnAction.setDisable(true);
                btnAction.setStyle("-fx-background-color: #D1FAE5; -fx-text-fill: #059669; -fx-font-weight: bold; -fx-background-radius: 10px;");
            } else {
                btnAction = new Button("💬 Ajouter Feedback"); btnAction.setPrefWidth(175); btnAction.setPrefHeight(42);
                btnAction.setStyle("-fx-background-color:#1e3a8a; -fx-text-fill:#ffffff; -fx-font-size: 13px; -fx-font-weight: bold; -fx-background-radius: 12px; -fx-cursor: hand; -fx-effect: dropshadow(gaussian, rgba(16,185,129,0.35), 10, 0, 0, 3);");
                btnAction.setOnAction(ev -> ouvrirFeedbackPourEntretien(e));
            }
        }

        Button btnSupprimer = new Button(" Supprimer"); btnSupprimer.setPrefWidth(165); btnSupprimer.setPrefHeight(42);
        if (estAnnule) {
            btnSupprimer.setStyle("-fx-background-color:#1e3a8a; -fx-text-fill:#ffffff; -fx-font-size: 13px; -fx-font-weight: bold; -fx-background-radius: 12px; -fx-cursor: hand; -fx-effect: dropshadow(gaussian, rgba(16,185,129,0.35), 10, 0, 0, 3);");
            btnSupprimer.setOnAction(ev -> supprimerEntretienHistorique(e));
        } else { btnSupprimer.setVisible(false); btnSupprimer.setManaged(false); }
    ...
    private Label buildStatutHistoriqueBadge(String statut, boolean hasFeedback) {
        ...
        if ("annulé".equals(statut)) { text = "❌ Annulé"; bgColor = "#FEE2E2"; textColor = "#DC2626"; }
        else if (hasFeedback) { text = "🏁 Réalisé  •  ✅ Feedback enregistré"; bgColor = "#D1FAE5"; textColor = "#059669"; }
        else { text = "🏁 Réalisé  •  ⏳ En attente de feedback"; bgColor = "#FEF3C7"; textColor = "#D97706"; }
        Label badge = new Label(text);
        badge.setStyle("-fx-background-color: " + bgColor + "; -fx-text-fill: " + textColor + "; -fx-font-size: 11px; -fx-font-weight: 700; -fx-padding: 3 10 3 10; -fx-background-radius: 20px;");
        return badge;
    }
    ...
    private void supprimerEntretienHistorique(Entretien e) {
        ...
        if (res.isPresent() && res.get() == ButtonType.OK) {
            try { service.delete(e.getIdEntretien()); chargerHistorique(); showAlert(Alert.AlertType.INFORMATION, "Suppression réussie", "L'entretien annulé a été supprimé définitivement."); }
            catch (SQLException ex) { showAlert(Alert.AlertType.ERROR, "Erreur", "Échec de la suppression : " + ex.getMessage()); }
        }
    }
    ...
    private void reorganiserEntretien(Entretien e) {
        try {
            ...
            stage.showAndWait(); chargerHistorique();
        } catch (IOException ex) { showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible d'ouvrir le formulaire de réorganisation : " + ex.getMessage()); }
    }

    private void ouvrirFeedbackPourEntretien(Entretien e) {
        if (!"réalisé".equals(e.getStatut())) { showAlert(Alert.AlertType.WARNING, "Action impossible", "Le feedback n'est disponible que pour les entretiens réalisés."); return; }
        try {
            ...
            stage.showAndWait(); chargerHistorique();
        } catch (IOException ex) { showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible d'ouvrir le formulaire de feedback : " + ex.getMessage()); }
    }

    // ════════════════════════════════════════════════════════════════
    //  NAVIGATION SIDEBAR
    // ════════════════════════════════════════════════════════════════
    @FXML private void ouvrirEntretiens  (ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/entretien-view.fxml",      "JobNest - Gestion des Entretiens", e); }
    @FXML private void ouvrirFeedbacks   (ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/feedback-interface.fxml",  "JobNest - Gestion des Feedbacks", e); }
    @FXML private void ouvrirCandidature (ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/GestionCandidatures.fxml", "JobNest - Gestion des Candidatures", e); }
    @FXML private void ouvrirProfil      (ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/profil-recruteur.fxml",    "JobNest - Mon Profil", e); }
    @FXML private void ouvrirOffresEmploi(ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/offre-emploi_view.fxml",   "JobNest - Offres d'Emploi", e); }
    @FXML private void ouvrirMatching    (ActionEvent e) { naviguerVers("/tn/jobnest/gentretien/matching-view.fxml",       "JobNest - Matching", e); }
