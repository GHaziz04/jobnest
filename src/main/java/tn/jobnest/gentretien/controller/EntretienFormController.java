...
import tn.jobnest.gentretien.service.GoogleCalendarReminderService;
...
public class EntretienFormController {
    ...
    private void initialize() {
        ...
        typeEntretien.valueProperty().addListener((obs, ov, nv) -> {
            boolean isP = "présentiel".equals(nv);
            boolean isV = "visio".equals(nv);
        ...
        lienVisio.textProperty().addListener((obs, ov, nv) -> {
            ...
            if (btnOuvrirMeet != null) {
                btnOuvrirMeet.setVisible(has);
                btnOuvrirMeet.setManaged(has);
            }
        });
    ...
    private void ouvrirLocationPicker() {
        try {
            ...
            showStyledAlert("error", "Erreur",
                    "Impossible d'ouvrir le sélecteur", ex.getMessage());
        }
    }
    ...
    public void setEntretien(Entretien e) {
        ...
        dateEntretien.setValue(
                e.getDateEntretien() != null ? e.getDateEntretien().toLocalDate() : null);
        heureDebut.setText(
                e.getHeureDebut() != null ? e.getHeureDebut().toLocalTime().toString() : "");
        heureFin.setText(
                e.getHeureFin() != null ? e.getHeureFin().toLocalTime().toString() : "");
        typeEntretien.setValue(e.getTypeEntretien());
        lieu.setText(e.getLieu());
        lienVisio.setText(e.getLienVisio());
        String cs = e.getStatut();
        if (cs != null && !statut.getItems().contains(cs))
            statut.getItems().add(cs);
        statut.setValue(cs);
        statut.setDisable(true);
    ...
    public void setEntretienPourReorganisation(Entretien e) {
        ...
        try {
            ...
            if (!participants.isEmpty()) {
                this.nomCandidat     = participants.get(0);
                this.idCandidatALier =
                        getCandidatIdByName(participants.get(0), e.getIdEntretien());
            }
        } catch (java.sql.SQLException ex) {
            System.err.println(
                    "Erreur chargement offre/participants (réorganisation) : "
                            + ex.getMessage());
        }
        ...
        if ("présentiel".equals(e.getTypeEntretien())) {
            ...
            lieu.clear();
            lienVisio.clear();
            lienVisio.setPromptText("Générez un nouveau lien Meet");
        }
    ...
    private int getCandidatIdByName(String nomComplet, int idEntretien) {
        try {
            ...
            java.sql.Connection conn =
                    tn.jobnest.gentretien.utils.MyDatabase.getInstance().getConn();
    ...
    private void genererLienMeet() {
        if (dateEntretien.getValue() == null) {
            showStyledAlert("warning", "Champ requis", "Date manquante",
                    "Veuillez sélectionner une date avant de générer le lien Meet.");
            return;
        }
        ...
        String hD = heureDebut.getText().trim();
        String hF = heureFin.getText().trim();
        ...
        try {
            ...
            showStyledAlert("error", "Format invalide",
                    "Format d'heure incorrect", "Utilisez HH:mm (ex: 14:30)");
        } catch (Exception ex) {
            showStyledAlert("error", "Erreur Meet",
                    "Impossible de générer le lien", ex.getMessage());
        }
    }
    ...
    private void validerAdresse() {
        ...
        if (adresse.isEmpty()) {
            showStyledAlert("warning", "Champ vide", "Aucune adresse saisie",
                    "Veuillez saisir ou sélectionner une adresse.");
            return;
        }
        try {
            String url = "https://www.google.com/maps/search/?api=1&query="
                    + java.net.URLEncoder.encode(adresse, "UTF-8");
            if (showStyledConfirm("Vérifier", "Ouvrir Google Maps ?",
                    "Adresse : " + adresse, "Oui, vérifier", "Non, continuer"))
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  SAVE — validation → popup récap → Google Calendar → BDD → fermeture
    // ─────────────────────────────────────────────────────────────────────────
    @FXML
    private void save() {
        ...
        String hF = heureFin.getText().trim();
        if (hD.isEmpty()) {
            showStyledAlert("warning", "Champ obligatoire",
        ...
        LocalTime debut, fin;
        try {
            debut = LocalTime.parse(hD);
            fin   = LocalTime.parse(hF);
        } catch (DateTimeParseException ex) {
            showStyledAlert("error", "Format invalide",
                    "Format d'heure incorrect", "Utilisez HH:mm (ex: 14:30)");
            return;
        ...
        try {
            ...
            showStyledAlert("error", "Erreur BD",
                    "Vérification impossible", ex.getMessage());
            return;
        }

        // ── Infos pour le popup récapitulatif ────────────────────────────
        ...
        String detailStr = estVisio
                ? lienVisio.getText().trim()
                : lieu.getText().trim();
        String mode = (entretien != null && entretien.getIdEntretien() != 0)
                ? (isReorganisation ? "réorganiser" : "modifier")
                : "créer";
        ...
        entretien.setLienVisio(
                lienVisio.getText() != null ? lienVisio.getText().trim() : "");
        entretien.setNoteRecruteur(
                noteRecruteur.getText() != null ? noteRecruteur.getText().trim() : "");
        ...
        try {
            RecruteurService recruteurService = new RecruteurService();
            Recruteur recruteur =
                    recruteurService.getRecruteurById(ID_RECRUTEUR_COURANT);
            if (recruteur != null && recruteur.getEmail() != null)
                emailRecruteur = recruteur.getEmail();
        } catch (Exception ex) {
            System.err.println("[Calendar] Erreur récupération email recruteur : "
                    + ex.getMessage());
        }

        // Récupérer l'email du candidat
        List<String> emailsCandidats = new ArrayList<>();
        try {
            if (idCandidatALier > 0) {
                CandidatService candidatService = new CandidatService();
                Candidat candidat =
                        candidatService.getCandidatById(idCandidatALier);
                if (candidat != null && candidat.getEmail() != null
                        && !candidat.getEmail().isBlank())
                    emailsCandidats.add(candidat.getEmail());
            } else {
                // Cas modification : récupérer les participants déjà liés
                List<String> nomsPart =
                        service.getParticipants(entretien.getIdEntretien());
                // On récupère les emails directement via une requête
                emailsCandidats = service.getParticipantEmails(
                        entretien.getIdEntretien());
            }
        } catch (Exception ex) {
            System.err.println("[Calendar] Erreur récupération email candidat : "
                    + ex.getMessage());
        }

        // Lieu ou lien selon le type
        String lieuOuLien = estVisio
                ? entretien.getLienVisio()
                : entretien.getLieu();

        // Titre de l'offre
        String titrePoste = titreOffreLiee.isEmpty()
                ? "Entretien JobNest"
                : titreOffreLiee;

        // Appel Google Calendar (non bloquant si erreur)
        String ancienEventId = entretien.getGoogleEventId();
        try {
            String newEventId = GoogleCalendarReminderService
                    .creerOuMettreAJourEvenement(
                            titrePoste,
                            entretien.getTypeEntretien(),
                            lieuOuLien,
                            entretien.getDateEntretien().toLocalDate(),
                            entretien.getHeureDebut().toLocalTime(),
                            entretien.getHeureFin().toLocalTime(),
                            emailRecruteur,
                            emailsCandidats,
                            ancienEventId   // null = création, sinon = mise à jour
                    );
            entretien.setGoogleEventId(newEventId);
            System.out.println("[JobNest] ✅ Événement Google Calendar : "
                    + newEventId);
        } catch (Exception ex) {
            // On ne bloque PAS la sauvegarde si Google Calendar échoue
            System.err.println("[JobNest] ⚠ Google Calendar non disponible : "
                    + ex.getMessage());
            // Conserver l'ancien ID en cas d'échec de mise à jour
            entretien.setGoogleEventId(ancienEventId);
        }
        ...
        try {
            ...
            showStyledAlert("error", "Erreur BD",
                    "Enregistrement impossible", ex.getMessage());
        }
    }
    ...
    private boolean showEntretienConfirmDialog(
            ...
            String date, String horaire,
            String type, String detail, String mode) {
        ...
        titleMain.setStyle(
                "-fx-font-size: 19px; -fx-font-weight: 800; -fx-text-fill: white;");
        Label titleSub = new Label("Vérifiez attentivement avant de " + mode);
        titleSub.setStyle(
                "-fx-font-size: 12px; -fx-text-fill: rgba(255,255,255,0.55);");
        ...
        infoStrip.setStyle(
                "-fx-background-color: rgba(255,255,255,0.09);" +
                        "-fx-background-radius: 10px; -fx-padding: 12 18 12 18;" +
        ...
        vOffre.setStyle(
                "-fx-font-size: 14px; -fx-font-weight: 700; -fx-text-fill: #60A5FA;");
        ...
        sectionAccent.setStyle(
                "-fx-background-color: #2563EB; -fx-background-radius: 2px;");
        Label sectionLabel = new Label("RÉCAPITULATIF DES INFORMATIONS");
        sectionLabel.setStyle(
                "-fx-font-size: 10px; -fx-font-weight: 800; -fx-text-fill: #94A3B8;");
        ...
        // Rappel Calendar (info)
        HBox calendarInfo = new HBox(8);
        calendarInfo.setAlignment(Pos.CENTER_LEFT);
        VBox.setMargin(calendarInfo, new Insets(6, 0, 0, 0));
        calendarInfo.setStyle(
                "-fx-background-color: #EFF6FF;" +
                        "-fx-background-radius: 10px; -fx-padding: 10 14 10 14;" +
                        "-fx-border-color: #BFDBFE; -fx-border-radius: 10px; -fx-border-width: 1;");
        Label calIcon  = new Label("📅");
        calIcon.setStyle("-fx-font-size: 13px;");
        Label calLabel = new Label(
                "Un rappel email sera envoyé 24h et 1h avant l'entretien "
                        + "au recruteur et au candidat.");
        calLabel.setStyle(
                "-fx-font-size: 11px; -fx-text-fill: #1E3A5F; -fx-wrap-text: true;");
        calLabel.setWrapText(true);
        calLabel.setMaxWidth(400);
        calendarInfo.getChildren().addAll(calIcon, calLabel);
        body.getChildren().add(calendarInfo);
        ...
        if (noteRecruteur != null && noteRecruteur.getText() != null
            ...
            noteRow.setStyle(
                    "-fx-background-color: #FFFBEB;" +
                            "-fx-background-radius: 10px; -fx-padding: 10 14 10 14;" +
                            "-fx-border-color: #FDE68A; -fx-border-radius: 10px;" +
                            "-fx-border-width: 1;");
            Label noteIcon = new Label("📝  ");
            noteIcon.setStyle("-fx-font-size: 13px;");
            Label noteKey = new Label("Note : ");
            noteKey.setStyle(
                    "-fx-font-size: 12px; -fx-font-weight: 700; -fx-text-fill: #92400E;");
        ...
        btnAnnuler.setOnMouseEntered(e -> btnAnnuler.setStyle(
                "-fx-background-color: #E2E8F0; -fx-text-fill: #334155;" +
                        "-fx-font-size: 13px; -fx-font-weight: 700;" +
                        "-fx-background-radius: 10px; -fx-cursor: hand;" +
                        "-fx-border-color: #94A3B8; -fx-border-width: 1.5;" +
                        "-fx-border-radius: 10px;"));
        ...
        btnConfirmer.setStyle(confirmBase);
        btnConfirmer.setOnMouseEntered(e -> btnConfirmer.setStyle(confirmHover));
        btnConfirmer.setOnMouseExited(e  -> btnConfirmer.setStyle(confirmBase));
    ...
    // ─────────────────────────────────────────────────────────────────────────
    private VBox buildRecapCard(String emoji, String labelTxt,
                                String valueTxt, boolean wide) {
        VBox card = new VBox(6);
        card.setAlignment(Pos.TOP_LEFT);
        card.setStyle(
                "-fx-background-color: white; -fx-background-radius: 12px;" +
                        "-fx-padding: 14 16 14 16; -fx-border-color: #EEF2FF;" +
                        "-fx-border-radius: 12px; -fx-border-width: 1;" +
        ...
        iconBg.setStyle(
                "-fx-background-color: #EFF6FF; -fx-background-radius: 8px;");
        ...
        lbl.setStyle(
                "-fx-font-size: 9px; -fx-font-weight: 800; -fx-text-fill: #94A3B8;");
        iconRow.getChildren().addAll(iconBox, lbl);

        Label val = new Label(valueTxt);
        val.setStyle(
                "-fx-font-size: 13px; -fx-font-weight: 700; -fx-text-fill: #1E3A5F;");
    ...
    private void showMeetSuccessDialog(String meetLink) {
        ...
        hdr.getChildren().addAll(
                ...
                new Label("Lien Google Meet généré !") {{
                    setStyle(
                            "-fx-font-size:17px;-fx-font-weight:800;-fx-text-fill:white;");
                }});
        ...
        bc.setOnAction(ev -> {
            ...
            javafx.scene.input.ClipboardContent cc2 =
                    new javafx.scene.input.ClipboardContent();
            cc2.putString(meetLink);
            cb.setContent(cc2);
            bc.setText("✅ Copié !");
            bc.setStyle(
        ...
        Button bo = new Button("🔗 Rejoindre");
        bo.setPrefHeight(40);
        bo.setStyle(
                "-fx-background-color:#2563EB;-fx-text-fill:white;" +
                        "-fx-font-weight:700;-fx-font-size:13px;-fx-background-radius:10;" +
                        "-fx-cursor:hand;" +
                        "-fx-effect:dropshadow(gaussian,rgba(37,99,235,0.4),8,0,0,3);");
        bo.setOnAction(ev -> {
            try { Desktop.getDesktop().browse(new URI(meetLink)); }
            catch (Exception ignored) {}
        });
        Button bco = new Button("Continuer");
        bco.setPrefHeight(40);
        bco.setStyle(
                "-fx-background-color:#F1F5F9;-fx-text-fill:#475569;" +
    ...
    //  POPUPS UTILITAIRES
    // =========================================================================
    private void showStyledAlert(String type, String title,
        ...
        switch (type) {
            ...
            default:        accent="#2563EB"; icon="ℹ"; bg="#EFF6FF";
        }
        VBox root = new VBox(0);
    ...
    private boolean showStyledConfirm(String title, String header,
                                      String body, String labelOui,
                                      String labelNon) {
        Stage d = new Stage();
        d.initModality(Modality.APPLICATION_MODAL);
        d.initStyle(StageStyle.UNDECORATED);
        ...
        tb.setPrefHeight(5);
        tb.setStyle(
                "-fx-background-color:#2563EB;-fx-background-radius:14px 14px 0 0;");
        HBox ct = new HBox(16);
        ct.setAlignment(Pos.TOP_LEFT);
        ct.setStyle("-fx-padding:20 24 16 24;-fx-background-color:#EFF6FF;");
        Label il = new Label("?");
        il.setStyle(
                "-fx-font-size:18px;-fx-font-weight:900;-fx-text-fill:#2563EB;" +
                        "-fx-background-color:#DBEAFE;-fx-background-radius:50%;" +
                        "-fx-min-width:36;-fx-min-height:36;-fx-alignment:CENTER;");
        VBox tc = new VBox(5);
        Label hl = new Label(header);
        hl.setStyle(
                "-fx-font-size:14px;-fx-font-weight:800;-fx-text-fill:#1E3A5F;");
        Label bl = new Label(body);
        bl.setStyle(
                "-fx-font-size:12px;-fx-text-fill:#64748B;-fx-wrap-text:true;");
        bl.setMaxWidth(280); bl.setWrapText(true);
        ...
        ft.setStyle(
                "-fx-padding:10 24 16 24;-fx-background-color:white;" +
                        "-fx-background-radius:0 0 14px 14px;");
        Button no = new Button(labelNon);
        no.setPrefHeight(38);
        no.setStyle(
                "-fx-background-color:#F1F5F9;-fx-text-fill:#475569;" +
                        "-fx-font-weight:600;-fx-font-size:12px;" +
                        "-fx-background-radius:9px;-fx-cursor:hand;");
        Button yes = new Button(labelOui);
        yes.setPrefHeight(38);
        yes.setStyle(
                "-fx-background-color:#2563EB;-fx-text-fill:white;" +
    ...
    private void makeDraggable(Stage stage, VBox root) {
        ...
        root.setOnMousePressed(ev -> {
            delta[0] = stage.getX() - ev.getScreenX();
            delta[1] = stage.getY() - ev.getScreenY();
        });
        root.setOnMouseDragged(ev -> {
            stage.setX(ev.getScreenX() + delta[0]);
            stage.setY(ev.getScreenY() + delta[1]);
        });
    }
}